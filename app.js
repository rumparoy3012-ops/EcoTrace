/**
 * EcoTrace - Carbon Footprint Tracker App Logic
 * Designed for dynamic responsive calculation, interactive SVG visualization, and personalized eco tips.
 */

// --- CONSTANTS & EMISSION FACTORS (kg CO2 per unit) ---
...
function initAppState() {
  ...
  // Verify if streak is broken (e.g. more than 1 day skipped)
  checkStreakValidity();
 ...
 * If the difference is > 1 day from today, reset streak.
 */
...
/**
 * Sync state inputs to both sliders and numeric text fields
 */
function updateInputUI() {
  // Commute
  elements.commuteSlider.value = state.inputs.commute;
  elements.commuteNumber.value = state.inputs.commute;
  updateSliderProgressFill(elements.commuteSlider, state.inputs.commute, 150);
  
  // Electricity
  elements.electricitySlider.value = state.inputs.electricity;
  elements.electricityNumber.value = state.inputs.electricity;
  updateSliderProgressFill(elements.electricitySlider, state.inputs.electricity, 50);
  
  // Transit
  elements.transitSlider.value = state.inputs.transit;
  elements.transitNumber.value = state.inputs.transit;
  updateSliderProgressFill(elements.transitSlider, state.inputs.transit, 100);
  
  elements.streakCount.innerText = state.streak;
}

/**
 * Calculates CSS linear gradients to simulate a filled slider track
 */
...
function updateCalculations() {
  ...
  elements.valCommute.innerText = `${commuteCO2.toFixed(1)} kg`;
  elements.valElectricity.innerText = `${electricityCO2.toFixed(1)} kg`;
  elements.valTransit.innerText = `${transitCO2.toFixed(1)} kg`;
  
  // 3. Update linear progress bar fills
  const maxCategoryEmissions = Math.max(commuteCO2, electricityCO2, transitCO2, 1); // Avoid division by zero
  elements.barCommute.style.width = `${(commuteCO2 / maxCategoryEmissions) * 100}%`;
  elements.barElectricity.style.width = `${(electricityCO2 / maxCategoryEmissions) * 100}%`;
  elements.barTransit.style.width = `${(transitCO2 / maxCategoryEmissions) * 100}%`;
  
  // 4. Update rating status badge
  updateStatusBadge(totalCO2);
  
  // 5. Redraw the SVG doughnut chart
...
/**
 * Updates status badge indicator
 */
function updateStatusBadge(totalCO2) {
  const ratingElement = elements.emissionStatus;
  ratingElement.className = 'emission-rating'; // Clear categories
...
/**
 * Draws and animates segments on the SVG doughnut chart
 */
function updateChart(commuteVal, electricityVal, transitVal, totalVal) {
  ...
  if (totalVal === 0) {
    return; // Leave chart empty except track
  }
  ...
  shares.forEach(share => {
    ...
    // Create new SVG circle element for this segment
    ...
    // Set dash array: length of segment followed by total circumference gap
    circle.setAttribute('stroke-dasharray', `${strokeDash} ${circumference}`);
    
    // Offset segment counter-clockwise so segments sit end-to-end
...
function generateAIAdvice(commuteVal, electricityVal, transitVal) {
  ...
  // Custom suggestion profiles
  if (max === commuteVal) {
    // Private commute is highest emitter
    ...
    // Electricity consumption is highest emitter
    ...
    // Public transport transit is highest emitter (already low per-km, but high volume)
...
/**
 * Handle updates to state and local storage when sliders or number inputs change
 */
function handleInputChange(category, val) {
...
/**
 * Bind HTML input events to synchronize Dual Slider / Number values
 */
function setupInputListeners() {
  // 1. Private Vehicle Commute
  elements.commuteSlider.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.commuteNumber.value = val;
    updateSliderProgressFill(elements.commuteSlider, val, 150);
    handleInputChange('commute', val);
  });
  
  elements.commuteNumber.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.commuteSlider.value = Math.min(val, 150); // Cap slider representation
    updateSliderProgressFill(elements.commuteSlider, Math.min(val, 150), 150);
    handleInputChange('commute', val);
  });
  
  // 2. Electricity
  elements.electricitySlider.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.electricityNumber.value = val;
    updateSliderProgressFill(elements.electricitySlider, val, 50);
    handleInputChange('electricity', val);
  });
  
  elements.electricityNumber.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.electricitySlider.value = Math.min(val, 50);
    updateSliderProgressFill(elements.electricitySlider, Math.min(val, 50), 50);
    handleInputChange('electricity', val);
  });
  
  // 3. Public Transit
  elements.transitSlider.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.transitNumber.value = val;
    updateSliderProgressFill(elements.transitSlider, val, 100);
    handleInputChange('transit', val);
  });
  
  elements.transitNumber.addEventListener('input', (e) => {
    const val = e.target.value;
    elements.transitSlider.value = Math.min(val, 100);
    updateSliderProgressFill(elements.transitSlider, Math.min(val, 100), 100);
    handleInputChange('transit', val);
  });
}

/**
 * Handle Streak Tracking Logging
 */
  ...
  elements.saveLogBtn.addEventListener('click', () => {
    ...
    if (state.lastLoggedDate) {
      ...
      if (lastLogged.getTime() === yesterday.getTime()) {
        ...
        state.streak = 1; // Reset to 1 since they broke it but logged today
      }
    } else {
      state.streak = 1; // First log ever
    }
